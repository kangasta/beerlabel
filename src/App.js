...
class App extends Component {
	constructor(props) {
		super(props);
		this.state = {'data': require('./APA.json').RECIPES.RECIPE}
	}

	render() {
		const toArray = a => (Array.isArray(a) ? a : [a]);
		const abv = Math.round((Number(this.state.data.OG._text)-Number(this.state.data.FG._text)) * 131.25 * 10)/10
		...
		const ibu = toArray(this.state.data.HOPS.HOP)
			.map(a => IBU(a.ALPHA._text/100, a.AMOUNT._text*1000, a.TIME._text, this.state.data.BATCH_SIZE._text, this.state.data.OG._text))
		...
		const ebc = Math.round(
			EBC(toArray(this.state.data.FERMENTABLES.FERMENTABLE)
			.map(a => MCU(a.COLOR._text, a.AMOUNT._text, this.state.data.BATCH_SIZE._text, ))
			.reduce((a,b) => Math.round(a+b))))

		return (
			<div className="App">
				<div className='Center'>
					<h1>{this.state.data.NAME._text}</h1>
					<h2>Metrics</h2>
					<ul>
						<li>ABV: {abv}</li>
						<li>IBU: {ibu}</li>
					...
					<h2>Fermentables</h2>
					<ul>
						{toArray(this.state.data.FERMENTABLES.FERMENTABLE).map(fermentable => (
							<li>{fermentable.NAME._text}: {Math.round(fermentable.AMOUNT._text*10)/10 + ' kg'}</li>
						))}
					</ul>
					<h2>Hops</h2>
					<ul>
						{toArray(this.state.data.HOPS.HOP).map(hop => (
							<li>{hop.NAME._text}: {hop.AMOUNT._text * 1e3} g, {Math.round(hop.TIME._text)} min</li>
						))}
					</ul>
					<h2>Yeasts</h2>
					<ul>
						{toArray(this.state.data.YEASTS.YEAST).map(yeast => (
